// Ponto único de leitura/escrita do localStorage para o fluxo de pagamento
// (Carrinho -> Método de Pagamento -> Selecionar Cartão / Pix).
//
// Únicas chaves utilizadas por todo o fluxo — nenhum outro módulo deve
// acessar 'locatem_pagamento_*' diretamente via localStorage, para evitar
// chaves soltas/duplicadas e manter o valor consistente entre as telas.
...
/**
 * Persiste o valor total da compra (calculado no Carrinho) como string
 * numérica com duas casas decimais e ponto como separador decimal
 * (ex.: "150.90"), conforme especificação do fluxo. Nunca inclui o
 * prefixo "R$" — a formatação em BRL é responsabilidade de quem exibe.
 */
...
/**
 * Lê o valor total persistido. Retorna 0 caso a chave esteja ausente ou
 * contenha um valor inválido (nunca deve ser recalculado a partir de
 * outras chaves — apenas o que foi salvo pelo Carrinho).
 */
...
/**
 * Persiste apenas dados não sensíveis do cartão usado no pagamento
 * (id, bandeira, últimos dígitos). Nunca deve receber número completo,
